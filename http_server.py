...
def response_ok(body=b"This is a minimal response", mimetype=b"text/plain"):
    ...
    return b"\r\n".join([
        ...
        b"", # Separate the head of the response and the body of the response
        body,
    ])
...
def response_not_found():
    ...
    return b"\r\n".join([
        ...
        b"",
        b"Sorry, we couldn't find what you were looking for.",
    ])
...
def response_path(path):
    ...
    full_path = os.path.dirname(os.path.abspath(__file__)) + "/webroot" + path

    # Raise name error if requested content is not present under webroot
    if not os.path.exists(full_path):
        raise NameError

    if os.path.isdir(full_path): # listdir returns a list containing the names in the directory
        mime_type = b"text/plain"
        content = "\r\n".join(os.listdir(full_path)).encode("utf-8")
    else:
        # mimetypes.guess_type guesses the type of file based on the path 
